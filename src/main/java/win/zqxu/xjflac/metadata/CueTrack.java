package win.zqxu.xjflac.metadata;
...
public class CueTrack {
  ...
  protected long offset; // Track offset in samples, relative to the beginning
                         // of the FLAC audio stream.
  protected byte number; // The track number.
  protected byte[] isrc = new byte[13]; // Track ISRC. This is a 12-digit
                                        // alphanumeric code plus a trailing
                                        // '\0'
  protected int type; // The track type: 0 for audio, 1 for non-audio.
  protected int preEmphasis; // The pre-emphasis flag: 0 for no pre-emphasis, 1
                             // for pre-emphasis.
  protected byte numIndices; // The number of track index points.
  protected CueIndex[] indices; // NULL if num_indices == 0, else pointer to
                                // array of index points.
