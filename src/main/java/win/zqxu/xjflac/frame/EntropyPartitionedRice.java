...
public class EntropyPartitionedRice extends EntropyCodingMethod {
  ...
  protected EntropyPartitionedRiceContents contents; // The context's Rice
                                                     // parameters and/or raw
                                                     // bits.
   ...
   * @param is
   *          The InputBitStream
   * @param predictorOrder
   *          The predicate order
   * @param partitionOrder
   *          The partition order
   * @param header
   ...
   * @param isExtended
   *          The RICE2 indicator flag
   * @throws IOException
   *           On error reading from InputBitStream
   */
  void readResidual(BitInputStream is, int predictorOrder, int partitionOrder, Header header,
      int[] residual, boolean isExtended) throws IOException {
    ...
    int pLen = isExtended ? ENTROPY_CODING_METHOD_PARTITIONED_RICE2_PARAMETER_LEN
        : ENTROPY_CODING_METHOD_PARTITIONED_RICE_PARAMETER_LEN;
    int pEsc = isExtended ? ENTROPY_CODING_METHOD_PARTITIONED_RICE2_ESCAPE_PARAMETER
    ...
    for (int partition = 0; partition < partitions; partition++) {
      ...
      if (riceParameter < pEsc) {
        ...
        int u = (partitionOrder == 0 || partition > 0) ? partitionSamples
