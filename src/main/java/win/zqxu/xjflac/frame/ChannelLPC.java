...
import win.zqxu.xjflac.ChannelData;
import win.zqxu.xjflac.LPCPredictor;
import win.zqxu.xjflac.io.BitInputStream;
import win.zqxu.xjflac.util.BitMath;
...
public class ChannelLPC extends Channel {
  ...
  private EntropyCodingMethod entropyCodingMethod; // The residual coding
                                                   // method.
  private int order; // The FIR order.
  private int qlpCoeffPrecision; // Quantized FIR filter coefficient precision
                                 // in bits.
  ...
  private int[] warmup = new int[MAX_LPC_ORDER]; // Warmup samples to prime the
                                                 // predictor, length == order.
   ...
   */
  public ChannelLPC(BitInputStream is, Header header, ChannelData channelData, int bps,
      int wastedBits, int order) throws IOException {
    ...
    // for (int i = 0; i < order; i++) System.out.println("Warm "+i+"
    // "+warmup[i]);
    ...
    case ENTROPY_CODING_METHOD_PARTITIONED_RICE:
      ...
      ((EntropyPartitionedRice) entropyCodingMethod).order = is
          .readRawUInt(ENTROPY_CODING_METHOD_PARTITIONED_RICE_ORDER_LEN);
      ((EntropyPartitionedRice) entropyCodingMethod).contents = channelData
    ...
    if (bps + qlpCoeffPrecision + BitMath.ilog2(order) <= 32) {
      if (bps <= 16 && qlpCoeffPrecision <= 16)
        LPCPredictor.restoreSignal(channelData.getResidual(), header.blockSize - order, qlpCoeff,
            order, quantizationLevel, channelData.getOutput(), order);
      else
        LPCPredictor.restoreSignal(channelData.getResidual(), header.blockSize - order, qlpCoeff,
            order, quantizationLevel, channelData.getOutput(), order);
    } else {
      LPCPredictor.restoreSignalWide(channelData.getResidual(), header.blockSize - order, qlpCoeff,
          order, quantizationLevel, channelData.getOutput(), order);
    }
  }
  ...
  public String toString() {
    ...
    sb.append(
        " qlpCoeffPrecision=" + qlpCoeffPrecision + " quantizationLevel=" + quantizationLevel);
    sb.append("\n\t\tqlpCoeff: ");
