...
import win.zqxu.xjflac.FixedPredictor;
...
public class ChannelFixed extends Channel {
  ...
  private int[] warmup = new int[MAX_FIXED_ORDER]; // Warmup samples to prime
                                                   // the predictor, length ==
                                                   // order.
  private int[] residual; // The residual signal, length == (blocksize minus
                          // order) samples.
   ...
   */
  public ChannelFixed(BitInputStream is, Header header, ChannelData channelData, int bps,
    ...
    FixedPredictor.restoreSignal(residual, header.blockSize - order, order, channelData.getOutput(),
        order);
  }
