...
public class Header {
   ...
   * frame header bytes, meaning everything before the CRC byte including the
   * sync code.
   */
  protected byte crc;
  ...
  public Header(BitInputStream is, byte[] headerWarmup, StreamInfo streamInfo)
    ...
    ByteData rawHeader = new ByteData(16); // MAGIC NUMBER based on the maximum
                                           // frame header size, including CRC
    ...
    for (int i = 0; i < 2; i++) {
      if (is.peekRawUInt(8) == 0xff) { // MAGIC NUMBER for the first 8 frame
                                       // sync bits
