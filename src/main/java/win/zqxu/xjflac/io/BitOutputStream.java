...
import win.zqxu.xjflac.util.CRC8;
...
public class BitOutputStream {
  ...
  private static final long[] MASK32 = new long[]{0, 0x0000000000000001, 0x0000000000000003,
      0x0000000000000007, 0x000000000000000F,
      0x000000000000001F, 0x000000000000003F, 0x000000000000007F, 0x00000000000000FF,
      0x00000000000001FF, 0x00000000000003FF,
      0x00000000000007FF, 0x0000000000000FFF, 0x0000000000001FFF, 0x0000000000003FFF,
      0x0000000000007FFF, 0x000000000000FFFF,
      0x000000000001FFFF, 0x000000000003FFFF, 0x000000000007FFFF, 0x00000000000FFFFF,
      0x00000000001FFFFF, 0x00000000003FFFFF,
      0x00000000007FFFFF, 0x0000000000FFFFFF, 0x0000000001FFFFFF, 0x0000000003FFFFFF,
      0x0000000007FFFFFF, 0x000000000FFFFFFF,
      0x000000001FFFFFFF, 0x000000003FFFFFFF, 0x000000007FFFFFFF, 0x00000000FFFFFFFF,
      0x00000001FFFFFFFFL,
      0x00000003FFFFFFFFL, 0x00000007FFFFFFFFL, 0x0000000FFFFFFFFFL, 0x0000001FFFFFFFFFL,
      0x0000003FFFFFFFFFL,
      0x0000007FFFFFFFFFL, 0x000000FFFFFFFFFFL, 0x000001FFFFFFFFFFL, 0x000003FFFFFFFFFFL,
      0x000007FFFFFFFFFFL,
      ...
      0x001FFFFFFFFFFFFFL,
      0x003FFFFFFFFFFFFFL, 0x007FFFFFFFFFFFFFL, 0x00FFFFFFFFFFFFFFL, 0x01FFFFFFFFFFFFFFL,
      0x03FFFFFFFFFFFFFFL,
      0x07FFFFFFFFFFFFFFL, 0x0FFFFFFFFFFFFFFFL, 0x1FFFFFFFFFFFFFFFL, 0x3FFFFFFFFFFFFFFFL,
      0x7FFFFFFFFFFFFFFFL,
      0xFFFFFFFFFFFFFFFFL};
  ...
  private boolean resize(int newCapacity) {
    ...
    System.arraycopy(buffer, 0, newBuffer, 0,
        Math.min(outBlurbs + ((outBits != 0) ? 1 : 0), newCapacity));
  ...
  public boolean concatenateAligned(BitOutputStream src) {
    ...
    if (outBits == 0) {
      ...
      buffer[outBlurbs] |= (src.buffer[src.consumedBlurbs]
          & ((1 << (BITS_PER_BLURB - outBits)) - 1));
  ...
  public void writeRawUInt(int val, int bits) throws IOException {
    ...
    // zero-out unused bits; WATCHOUT: other code relies on this, so this needs
    // to stay
    ...
    while (bits > 0) {
      ...
      if (n == BITS_PER_BLURB) { // i.e. outBits == 0
        if (bits < BITS_PER_BLURB) {
          ...
          // we know k < 32 so no need to protect against the gcc bug mentioned
          // above
          val &= (~(0xffffffff << k));
          bits -= BITS_PER_BLURB;
        }
        ...
        // we know n > 0 so k < 32 so no need to protect against the gcc bug
        // mentioned above
        val &= (~(0xffffffff << k));
        bits -= n;
        outBlurbs++;
    ...
    while (bits > 0) {
      if (outBits == 0) {
        if (bits < BITS_PER_BLURB) {
          ...
          // we know k < 64 so no need to protect against the gcc bug mentioned
          // above
        ...
        // we know n > 0 so k < 64 so no need to protect against the gcc bug
        // mentioned above
        val &= (~(0xffffffffffffffffL << k));
        bits -= n;
        outBits += n;
  ...
  public void writeRawUIntLittleEndian(int val) throws IOException {
    // NOTE: we rely on the fact that write_raw_uint32() masks out the unused
    // bits
    writeRawUInt(val, 8);
   ...
   * |= val_bits; pattern < <= val_bits; pattern |= (val & ((1 < < val_bits) -
   * 1)); if (!write_raw_uint32(bb, pattern, total_bits)) return false; } else {
   * // write the '-0' escape code first if (!write_raw_uint32(bb, 3 u < <
   * parameter, 2 + parameter)) return false; // write the length if
   * (!write_raw_uint32(bb, val_bits, 5)) return false; // write the value if
   * (!write_raw_int32(bb, val, val_bits)) return false; } return true; } #
   * endif // ifdef SYMMETRIC_RICE
   */
   ...
   * 
   * @throws IOException
   *           On error writing to bit stream
   */
   ...
   * unsigned * bytes, void * client_data), void * client_data) { uint32 sign =
   * 0, lsbs = 0, msbs = 0;
   * 
   ...
   * client_data)) return false; // read the sign bit if
   * (!read_bit_to_uint32(bb, & sign, read_callback, client_data)) return false;
   * // read the binary LSBs if (!read_raw_uint32(bb, & lsbs, parameter,
   * read_callback, client_data)) return false; // compose the value val = (msbs
   * < < parameter) | lsbs; if (sign) val = - (* val);
   * 
   ...
   * ASSERT(0 != bb); ASSERT(0 != buffer); ASSERT(parameter <= 31); // read the
   * unary MSBs and end bit if (!read_unary_unsigned(bb, & msbs, read_callback,
   * client_data)) return false; // read the binary LSBs if
