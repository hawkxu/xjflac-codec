...
import win.zqxu.xjflac.frame.BadHeaderException;
import win.zqxu.xjflac.frame.ChannelConstant;
import win.zqxu.xjflac.frame.ChannelFixed;
import win.zqxu.xjflac.frame.ChannelLPC;
import win.zqxu.xjflac.frame.ChannelVerbatim;
import win.zqxu.xjflac.frame.Frame;
import win.zqxu.xjflac.frame.Header;
import win.zqxu.xjflac.io.BitInputStream;
import win.zqxu.xjflac.io.RandomFileInputStream;
import win.zqxu.xjflac.metadata.Application;
import win.zqxu.xjflac.metadata.CueSheet;
import win.zqxu.xjflac.metadata.Metadata;
import win.zqxu.xjflac.metadata.Padding;
import win.zqxu.xjflac.metadata.Picture;
import win.zqxu.xjflac.metadata.SeekPoint;
import win.zqxu.xjflac.metadata.SeekTable;
import win.zqxu.xjflac.metadata.StreamInfo;
import win.zqxu.xjflac.metadata.Unknown;
import win.zqxu.xjflac.metadata.VorbisComment;
import win.zqxu.xjflac.util.ByteData;
import win.zqxu.xjflac.util.CRC16;
...
public class FLACDecoder {
  ...
  private byte[] headerWarmup = new byte[2]; // contains the sync code and
                                             // reserved bits
  // private int state;
   ...
   *
   * @param frame
   *          the frame to send to the PCM processors
   * @param pcmData
   ...
   * 
   * @param samplesAbsolute
   *          The number of samples from start of file to seek past
   * @return The new sample position in the file (from which the next frame will
   *         be read)
   * @throws IOException
   */
  public long seek(long samplesAbsolute) throws IOException {
    ...
    // track seek delta (difference between requested and current position in
    // samples)
    ...
    // use data start and end positions as initial lower and upper bounds for
    // seeking
    // Note: should be at first data frame immediately after readMetadata()
    findFrameSync(); // this reads up to the 2 sync bytes of the first data
                     // frame into bitStream
    // keep track of current frame starting position
    long bytePositionCurrentFrame = bitStream.getTotalBytesRead() - 2;
    // Note: uses virtual temporary SeekPoint variables to store sample/byte
    // offset pairs
    ...
    // Note: maximum frame size used to increase chance of being before final
    // frame
    ...
    if (!(seekTable == null)) {
      ...
      for (int i = 0; i < seekTable.numberOfPoints(); i++) {
        ...
        // check against requested sample position and current lower and upper
        // bounds
        if (currentSeekPoint.getSampleNumber() < samplesAbsolute) {
          ...
          if (currentSeekPoint.getSampleNumber() > beforeSeekPosition.getSampleNumber()) {
            ...
            if (samplesAbsolute < beforeSeekPosition.getSampleNumber()
              ...
              afterSeekPosition = currentSeekPoint; // set as new upper bound as
                                                    // well
    ...
    // use lower and upper bounds to estimate position of frame containing
    // sample position:
    ...
    // Note: if it had been found, then upper (after) would equal lower (before)
    // sample position
    if (afterSeekPosition.getSampleNumber() > beforeSeekPosition.getSampleNumber()) {
      ...
      // remove the frame size to seek to before start of frame containing
      // sample position
    ...
    /*
     * loop until at requested sample position: 0. starts at
     * bytePositionEstimate from above initial logic; 1. find and read the next
     * frame from the current location; 2. get frame sample range: (a) if
     * requested sample position > current frame sample range, goto 1.; (b) if
     * requested sample position < current frame sample range, seek backwards
     * and goto 1.; (to prevent infinite loops, seeks further backwards on each
     * time 2.(b) is reached) (c) if requested sample position within current
     * frame sample range, exit loop.
     */
    // Note: exits loop when current frame contains requested sample position
    int framesToSeekBack = 1; // number of frames to go back if past requested
                              // position
    ...
    while (!(seekDelta == 0)) // Note: this condition is not currently used
    {
      ...
      findFrameSync(); // positions 2 bytes into the frame (as reads the 2 sync
                       // bytes)
      ...
      if (frame == null) {
        ...
        if (eof) {
          // end of file: attempt to seek back to previous frame (or further if
          // required)
   ...
   * 
   * @param bytePositionEstimate
   *          The next position to seek to (in bytes)
   * @param framesToSeekBack
   *          The number of frames to seek backwards
   * @param estimatedFrameSize
   ...
   * // above function sets the status for us break; case
   * DECODER_SEARCH_FOR_FRAME_SYNC : frameSync(); // above function sets the
   * status for us //System.exit(0); break; case DECODER_READ_FRAME :
   * readFrame(); break; case DECODER_END_OF_STREAM : case DECODER_ABORTED :
    ...
    if (frameCRC == frame.getCRC()) {
      ...
      case Constants.CHANNEL_ASSIGNMENT_LEFT_SIDE:
        for (i = 0; i < frame.header.blockSize; i++)
          channelData[1].getOutput()[i] = channelData[0].getOutput()[i]
              - channelData[1].getOutput()[i];
        break;
      case Constants.CHANNEL_ASSIGNMENT_RIGHT_SIDE:
        for (i = 0; i < frame.header.blockSize; i++)
  ...
  private void readSubframe(int channel, int bps) throws IOException, FrameDecodeException {
    ...
    if ((x & 0x80) != 0) {
      ...
      frame.subframes[channel] = new ChannelConstant(bitStream, frame.header, channelData[channel],
          bps, wastedBits);
    } else if (x == 2) {
      frame.subframes[channel] = new ChannelVerbatim(bitStream, frame.header, channelData[channel],
          bps, wastedBits);
    } else if (x < 16) {
      // state = DECODER_UNPARSEABLE_STREAM;
      throw new FrameDecodeException(
          "ReadSubframe Bad Subframe Type: " + Integer.toHexString(x & 0xff));
    } else if (x <= 24) {
      // FLACSubframe_Fixed subframe = read_subframe_fixed_(channel, bps, (x >>
      // 1) & 7);
      frame.subframes[channel] = new ChannelFixed(bitStream, frame.header, channelData[channel],
          bps, wastedBits, (x >> 1) & 7);
    } else if (x < 64) {
      // state = DECODER_UNPARSEABLE_STREAM;
      throw new FrameDecodeException(
          "ReadSubframe Bad Subframe Type: " + Integer.toHexString(x & 0xff));
    } else {
      frame.subframes[channel] = new ChannelLPC(bitStream, frame.header, channelData[channel], bps,
          wastedBits, ((x >> 1) & 31) + 1);
    }
    if (haveWastedBits) {
      int i;
