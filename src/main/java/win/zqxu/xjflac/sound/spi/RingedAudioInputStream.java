...
import win.zqxu.xjflac.util.RingBuffer;
...
public abstract class RingedAudioInputStream extends AudioInputStream {
   ...
   * <p>
   * This value is always in the range 0 through <code>count</code>. If it is
   ...
   * bytes to be read and filtered from the underlying inputstream.
   * 
   * @see #buf
   */
  // protected int pos;
   ...
   * <p>
   * This value is always in the range -1 through <code>pos</code>. If there is
   * no marked position in the inputstream, this field is -1. If there is a
   * marked position in the inputstream, then <code>buf[markpos]</code> is the
   * first byte to be supplied as input after a <code>reset</code> operation. If
   * <code>markpos</code> is not -1, then all bytes from positions
   ...
   * array, with suitable adjustments to the values of
   ...
   * <code>prebuf[count-1]</code> contain buffered input data obtained from the
   ...
   * <p>
   * This value is always in the range 0 through <code>precount</code>. If it is
   * less than <code>precount</code>, then <code>prebuf[pos]</code> is the next
   * byte to be supplied as input. If it is equal to <code>precount</code>, then
   * the next <code>read</code> or <code>skip</code> operation will require more
   * bytes to be read from the contained inputstream.
   * 
   * @see #prebuf
   */
  ...
  /**
   * Creates a <code>RingedAudioInputStream</code> and saves its argument, the
  ...
  /**
   * Creates a <code>RingedAudioInputStream</code> with the specified buffer
   * size, and saves its argument, the inputstream <code>in</code> for later
   ...
   */
  /*
   * protected void makeSpace() { if (markpos < 0) pos = 0; // no mark: throw
   * away the buffer else if (pos >= buf.length) // no room left in buffer if
   * (markpos > 0) { // can throw away early part of the buffer int sz = pos -
   * markpos; System.arraycopy(buf, markpos, buf, 0, sz); pos = sz; markpos = 0;
   * } else if (buf.length >= marklimit) { markpos = -1; // buffer got too big,
   * invalidate mark pos = 0; // drop buffer contents } else { // grow buffer
   * int nsz = pos * 2; if (nsz > marklimit) nsz = marklimit; byte[] nbuf = new
   * byte[nsz]; System.arraycopy(buf, 0, nbuf, 0, pos); buf = nbuf; } count =
   * pos; }
   */
   ...
   * buffer (<code>count - pos</code>). The result of calling the
   * <code>available</code> method of the underlying inputstream is not used, as
   * this data will have to be filtered, and thus may not be the same size after
   * processing (although subclasses that do the filtering should override this
   * method and use the amount of data available in the underlying inputstream).
   * 
   ...
   * invalidated), an <code>IOException</code> is thrown. Otherwise,
