...
public class FlacFormatConversionProvider extends FormatConversionProvider {
   ...
   * 
   * @param notSpecifiedOK
   *          if true, bitSize and channels can have value
  ...
  private AudioFormat[] getTargetFormats(AudioFormat.Encoding targetEncoding,
    ...
    // // encode to FLAC
    // if (DEBUG) {
    // System.out.println("FLAC converter: can encode: " + sourceFormat + " to "
    // + targetEncoding);
    // }
    // AudioFormat[] formats = { //
    ...
    if (bitSizeOK && channelsOK
      ...
      if (DEBUG) {
        System.out
            .println("FLAC converter: cannot de/encode: " + sourceFormat + " to " + targetEncoding);
      }
      AudioFormat[] formats = {};
      return formats;
    }
   ...
   * @return stream from which processed data with the specified target encoding
   *         may be read.
   */
