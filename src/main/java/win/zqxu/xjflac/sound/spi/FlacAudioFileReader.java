...
import win.zqxu.xjflac.Constants;
...
public class FlacAudioFileReader extends AudioFileReader {
  ...
  public AudioFileFormat getAudioFileFormat(InputStream stream)
    ...
    if (!stream.markSupported()) {
      // see AudioSystem#getAudioFileFormat(InputStream stream) javadocs for
      // contract
      throw new IOException("InputStream must support mark(), but doesn't: " + stream);
    }
    stream.mark(256); // should be more than enough for the magic FLAC header,
                      // see FLACDecoder#readStreamSync()
    try {
  ...
  protected AudioFileFormat getAudioFileFormat(InputStream bitStream, int mediaLength)
    ...
    /*
     * if (!OGGID.equals(new String(header, 0, 4))) { throw new
     * UnsupportedAudioFileException( "missing ogg id!"); } // how many segments
     * are there? segments = header[SEGOFFSET] & 0xFF; if (segments > 1) { throw
     * new UnsupportedAudioFileException(
     * "Corrupt Flac Header: more than 1 segments"); } dis.readFully(header,
     * OGG_HEADERSIZE, segments); baos.write(header, OGG_HEADERSIZE, segments);
     * chksum = OggCrc.checksum(chksum, header, OGG_HEADERSIZE, segments); //
     * get the number of bytes in the segment bodybytes = header[OGG_HEADERSIZE]
     * & 0xFF; if (bodybytes != FLAC_HEADERSIZE) { throw new
     * UnsupportedAudioFileException( "Corrupt Flac Header: size=" + bodybytes);
     * } // read the Flac header dis.readFully(header, OGG_HEADERSIZE + 1,
     * bodybytes); baos.write(header, OGG_HEADERSIZE + 1, bodybytes); chksum =
     * OggCrc.checksum(chksum, header, OGG_HEADERSIZE + 1, bodybytes); // make
     * sure its a Flac header if (!FLACID.equals(new String(header,
     * OGG_HEADERSIZE + 1, 8))) { throw new UnsupportedAudioFileException(
     * "Corrupt Flac Header: missing Flac ID"); } mode = readInt(header,
     * OGG_HEADERSIZE + 1 + 40); sampleRate = readInt(header, OGG_HEADERSIZE + 1
     * + 36); channels = readInt(header, OGG_HEADERSIZE + 1 + 48); int nframes =
     * readInt(header, OGG_HEADERSIZE + 1 + 64); boolean vbr = readInt(header,
     * OGG_HEADERSIZE + 1 + 60) == 1; // Checksum if (chksum != origchksum)
     * throw new IOException("Ogg CheckSums do not match"); // Calculate
     * frameSize if (!vbr) { // Frames size is a constant so: // Read Comment
     * Packet the Ogg Header of 1st data packet; // the array table_segment
     * repeats the frame size over and over. } // Calculate frameRate if (mode
     * >= 0 && mode <= 2 && nframes > 0) { frameRate = ((float) sampleRate) /
    ...
    try {
      ...
      // bitStream.reset();
      // }
      final UnsupportedAudioFileException unsupportedAudioFileException = new UnsupportedAudioFileException(
          ioe.getMessage());
    ...
    if (streamInfo.getSampleRate() > 0) {
      final long duration = (streamInfo.getTotalSamples() * 1000L * 1000L)
  ...
  public AudioInputStream getAudioInputStream(final InputStream stream)
      throws UnsupportedAudioFileException, IOException {
    if (!stream.markSupported()) {
      // see AudioSystem#getAudioInputStream(InputStream stream) javadocs for
      // contract
      throw new IOException("InputStream must support mark(), but doesn't: " + stream);
    }
    stream.mark(256); // should be more than enough for the magic FLAC header,
  ...
  protected AudioInputStream getAudioInputStream(InputStream inputStream, int medialength)
    ...
    int frameLength = medialength; // if frameSize not specified, use byte
                                   // length, see AudioInputStream
