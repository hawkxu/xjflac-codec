...
public class WavWriter {
   ...
   *          The output sream
   * @param totalSamples
   *          The total samples to be written
   * @param channels
   *          Number of channels
   * @param bps
  ...
  public void writeHeader() throws IOException {
    ...
    // if (dataSize >= 0xFFFFFFDC) throw new IOException("ERROR: stream is too
    // big to fit in a single file chunk (Datasize="+dataSize+")");
    ...
    osLE.writeInt(sampleRate * channels * ((bps + 7) / 8)); // or is it
        ...
        if (isUnsignedSamples) {
          for (sample = wideSample = 0; wideSample < wideSamples; wideSample++)
            for (channel = 0; channel < channels; channel++) {
              // System.out.print("("+(int)((byte)(channelData[channel].getOutput()[wideSample]
              // + 0x80))+")");
